...
module.exports = (client, message) => {
    ...
    if (mp.search(/mp ph111|miniprojet ph111|projet ressort|miniprojet ressort|mp ressort|mini projet resort|mini projet ph111/i) !== -1) {
        ...
        message.channel.send("```yaml\nAnnee 2019-2020- partager par: Zakaria =>```")                                 //1
            .then(() => message.channel.send({files: ['./aero1Sources/ph11/mph11-2019Zak.pdf/']}))
            .then(() => message.channel.send("```yaml\nAnnee 2018-2019- partager par: Elena =>``` "))                     //2
            .then(() => message.channel.send({files: ['./aero1Sources/ph11/mph11-2018Elena.pdf/']}))
            .then(() => message.channel.send("```yaml\nAnnee 2017-2018- partager par: Valentin =>``` "))                   //3
            .then(() => message.channel.send({files: ['./aero1Sources/ph11/mph11-2017Valentin.pdf/']}))
    ...
    //ph121 = pendule
        ...
        message.channel.send("```yaml\nAnnee 2018-2019- partager par: Elena =>```")                                 //1
            .then(() => message.channel.send({files: './aero1Sources/ph12/pendule/pendule2018-Elena.pdf/'}))
            .then(() => message.channel.send("```yaml\nAnnee 2012-2013 partager par: Thomas =>``` "))                     //2
            .then(() => message.channel.send({files: './aero1Sources/ph12/pendule/pendule2012-Thomas.pdf/'}))
        message.channel.stopTyping()
    ...
    //ph121 = optique
        ...
        message.channel.send("```yaml\nAnnee 2019-2020- partager par: Jeremie et Zakaria =>```")                                 //1
            .then(() => message.channel.send({files: './aero1Sources/ph12/optique/tpOptique2020-Jeremie_Zakaria.pdf/'}))
            .then(() => message.channel.send("```yaml\nAnnee 2019-2020- partager par: Lea =>```"))                     //2
            .then(() => message.channel.send({files: './aero1Sources/ph12/optique/tpOptique2020-Lea.pdf/'}))
        message.channel.stopTyping()
    ...
    //ph121 = calorimetrie
        ...
        message.channel.send("```yaml\nAnnee 2018-2019- partager par: Zakaria et Jeremie => =>```")                             //1
            .then(() => message.channel.send({files: './aero1Sources/ph12/calorimetrie/calorimetrie2019-Jeremie-Zakaria.pptx/'}))
            .then(() => message.channel.send("```yaml\nAnnee 2019-2020- partager par: Lea```"))                        //2
            .then(() => message.channel.send({files: './aero1Sources/ph12/calorimetrie/calorimetrie2019-Lea.pptx/'}))
            .then(() => message.channel.send("```yaml\nAnnee 2019-2020- partager par: Elena```"))                     //3
            .then(() => message.channel.send({files: './aero1Sources/ph12/calorimetrie/calorimetrie2018-Elena.pdf/'}))
                ...
                underCollector.on('collect', message => {
                    ...
                    if (message.content === ("comete" || "halley" || "comete halley" || "mini projet comète de Halley" || "comète de Halley")) {
                        ...
                        message.channel.send("```yaml\nAnnee 2019-2020- partager par: Auriane => ```")                                  //1
                            .then(() => message.channel.send({files: './aero1Sources/ph12/comete/comete2020-Auriane_Zakaria.pdf'}))
                            .then(() => message.channel.send("```yaml\nAnnee 2019-2020- partager par: Romain => ```"))                    //2
                            .then(() => message.channel.send({files: './aero1Sources/ph12/comete/comete2020-Romain.pdf/'}))
                            .then(() => message.channel.send("```yaml\nAnnee 2019-2020- partager par: Lea => ```"))                    //3
                            .then(() => message.channel.send({files: './aero1Sources/ph12/comete/comete2020-Lea.pdf/'}))
                            .then(() => message.channel.send("```yaml\nAnnee 2019-2020- partager par: Graisth => ```"))                    //4
                            .then(() => message.channel.send({files: './aero1Sources/ph12/comete/comete2020-Graisth.pdf/'}))
                            .then(() => message.channel.send("```yaml\nAnnee 2018-2019- partager par: Baptiste => ```"))                    //5
                            .then(() => message.channel.send({files: './aero1Sources/ph12/comete/comete2019-Baptiste.pdf/'}))
                            .then(() => message.channel.send("```yaml\nAnnee 2018-2019- partager par: Elena => ```"))                       //6
                            .then(() => message.channel.send({files: './aero1Sources/ph12/comete/comete2019-Elena.pdf/'}))
                    }
                    ...
                    //ELEC
                        ...
                        message.reply("je t envoi ca de suite")
                        message.channel.send("```yaml\nAnnee 2019-2020- partager par: Jeremie et Zakaria =>```")                                 //1
                            .then(() => message.channel.send({files: './aero1Sources/elec/MP_ELEC2020-Zakaria_Jeremie.pdf/'}))
                            .then(() => message.channel.send("```yaml\nAnnee 2018-2019- partager par: Elena =>```"))
                            .then(() => message.channel.send({files: './aero1Sources/elec/Mini_projet_elec_Elena2018.pdf/'}))
    ...
    if (tpGMTrois.indexOf("genie maths tp3" || "genie math tp3" || "tp3 newton" || "methode de newton" || "tp3 methode de newton") !== -1) {
        ...
        message.channel.send({files: './aero1Sources/GenieMaths/tp3.pdf/'})                                       //1
        message.channel.stopTyping()
    }
    ...
    if (elecUn.indexOf("miniprojet elec" || "mini projet elec" || "mp elec") !== -1) {
        ...
        message.channel.send("```yaml\nAnnee 2019-2020- partager par: Jeremie et Zakaria =>```")                                 //1
            ...
            .then(() => message.channel.send({files: './aero1Sources/elec/Mini_projet_elec_Auriane.pdf/'}))
            .then(() => message.channel.send("```yaml\nAnnee 2019-2020- partager par: Bastien =>```"))                    //2
            .then(() => message.channel.send({files: './aero1Sources/elec/Mini_projet_elec_Bastien.pdf/'}))
            .then(() => message.channel.send("```yaml\nAnnee 2018-2019- partager par: Elena =>```"))                    //2
            .then(() => message.channel.send({files: './aero1Sources/elec/Mini_projet_elec_Elena2018.pdf/'}))
    ...
    if (elecDeux.indexOf("tp pspice 2") !== -1) {
        ...
        message.reply("je t envoi ca de suite")
        message.channel.send("```yaml\nAnnee 2019-2020- partager par: Zakaria =>```")                                 //1
            .then(() => message.channel.send({files: './aero1Sources/tp/elec/TP2020_PSPICE_2-Zak.pdf/'}))
        message.channel.stopTyping()
    }
        ...
        collector2.on('collect', message => {
            if ((message.content).toLowerCase() === "genie maths") {
                message.channel.send("c'est tout bon pour moi je t'envoi ca")
                    ...
                    .then(() => message.channel.send({files: './aero1Sources/tp/GenieMaths/tp3.pdf/'}))
                ...
                message.channel.send("Recu chef")
                    ...
                    .then(() => message.channel.send({files: './aero1Sources/tp/elec/TP2020_PSPICE_1-Auriane.pdf/'}))
